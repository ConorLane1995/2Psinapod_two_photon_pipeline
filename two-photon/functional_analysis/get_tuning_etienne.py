    ...
    for freq in cell_traces:
        ...
        for intensity in cell_traces[freq]:
            ...
            for trial in cell_traces[freq][intensity]:
                ...
                baseline_auc = np.trapz(baseline)/len(baseline)

                response = trace[n_baseline_frames:]
                adjusted_response = np.trapz(response)/len(response) - baseline_auc
                # print(zscore_response)
                # input()
                all_trials_of_this_intensity.append(adjusted_response)
            ...
            tuning_curves[frequency_counter,intensity_counter] = response
