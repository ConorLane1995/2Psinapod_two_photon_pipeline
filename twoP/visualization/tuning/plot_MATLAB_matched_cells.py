...
"""
def plot_tuning_traces(cell_traces,n_frequencies,n_intensities,y_limit,cell_ID):
...
def main():
    ...
    cell_matches_dict = scio.loadmat(BASE_PATH_2 + CELL_MATCH_FILE)
    cell_matches = cell_matches_dict["cell_matches"]
    
    # load the first dictionary file
    with open(BASE_PATH_1 + CELL_DICT, 'rb') as f:
        cell_dict_1 = pickle.load(f)

     # load the second dictionary file
    with open(BASE_PATH_2 + CELL_DICT, 'rb') as f:
        cell_dict_2 = pickle.load(f)

    # load the first recording info file - assumes both recordings have same frequencies and intensities presented.
    with open(BASE_PATH_1 + "recording_info.pkl","rb") as f:
        recording_info_1 = pickle.load(f)

    frequencies = recording_info_1['frequencies']
    intensities = recording_info_1['intensities']

    # Bring up the cell IDs of the pair the MATLAB script identified. 
    cell_pair = np.array(cell_matches[MATCHED_CELL_INDEX,:])


    plot_single_tuning_curve(cell_dict_1[cell_pair[0]]['tuning'],cell_pair[0],frequencies,intensities)
    plot_tuning_traces(cell_dict_1[cell_pair[0]]['traces'],len(frequencies),len(intensities),Y_LIMIT,cell_pair[0])
    plot_single_tuning_curve(cell_dict_2[cell_pair[1]]['tuning'],cell_pair[1],frequencies,intensities)
    plot_tuning_traces(cell_dict_2[cell_pair[1]]['traces'],len(frequencies),len(intensities),Y_LIMIT,cell_pair[1])
    plt.show()
